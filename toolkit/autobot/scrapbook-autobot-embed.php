...
if(isset($_GET['uid'])){
    ...
    $query = mysql_query("SELECT * from sbab_users WHERE uid='".$_GET['uid']."'");
    ...
    if ($orkut_state[0] != NULL)
    {
        ...
        if ($uid != $_GET['uid']) {
            ...
            mysql_query("INSERT INTO `op_toolkit_db`.`sbab_scraps` (`sno`, `user`, `scrap`, `pass`) VALUES (NULL, '{$user}', '{$msg}', '{$pass}')");
            if ($reply != NULL){
