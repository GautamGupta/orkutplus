...
if($_GET['catid'] != NULL){
    ...
    if($_GET['page'] == NULL){
        redirect_to("http://scraps.orkutplus.net/{$catid}/{$catinfo['category']}/1");
    }else{
        $page = $_GET['page'];
    }
    if(isset($_GET['cat']) && !is_null($_GET['cat']) && $_GET['cat'] == $catinfo['category']){
        ...
        if($page > $co){
            redirect_to("http://scraps.orkutplus.net/{$catid}/{$catinfo['category']}/{$co}");
        }
    }else{
        redirect_to("http://scraps.orkutplus.net/{$catid}/{$catinfo['category']}/{$page}");
    }
}else{
    redirect_to("http://scraps.orkutplus.net/404.php");
}
...
<h2 class="title"><a href="http://scraps.orkutplus.net/<?php echo $catid; ?>/<?php echo $catinfo['category']; ?>/1"><?php echo $catinfo['category_name']; ?> Glitter Scraps and Graphics<?php if($co != 1){ echo " - Page ".$page; } ?></a></h2>
</div>
...
if($co > 1){
    ...
    for($pf=1;$pf<=$co;$pf++){
        if($pf == $page){
            ...
            echo '<a href="http://scraps.orkutplus.net/'.$catid.'/'.$catinfo['category_name'].'/'.$pf.'" title="Page Number '.$pf.' for Category '.$catinfo['category_name'].'">'.$pf.'</a> ';
        }
    }
    echo "</div></center></div>";
}
