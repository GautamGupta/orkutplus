...
for($cf=0;$cf<=$countcats;$cf++){
    echo '<p align="left"><span><a href="http://scraps.orkutplus.net/'.$listcats[$cf]["id"].'/'.$listcats[$cf]["category"].'/1" title="'.$listcats[$cf]['category_name'].' Glitter Scraps and Graphics">&#9679; '.$listcats[$cf]["category_name"].'</a> <dfn title="There are '.$listcats[$cf]["number_of_imgs"].' Images in the Category - '.$listcats[$cf]["category_name"].'">('.$listcats[$cf]["number_of_imgs"].')</dfn></span></p>';
}
?>
</div></div><br/>
</div><!-- /rbcontent -->
