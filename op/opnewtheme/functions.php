...
function getThumbnailURL($postId) {
    if( get_post_meta($postId, "Thumbnail", true) ) {

    }else{  
        $images =& get_children('post_type=attachment&post_mime_type=image&post_parent=' . $postId);
        if(!empty($images)){
            $firstImageSrc = wp_get_attachment_image_src($images[0]);
            //if(strlen($firstImageSrc[0])) {
                add_post_meta($postId, 'Thumbnail', $firstImageSrc[0]);
            /*}else{
                $sThumbUrl = "http://www.kisaso.com/wp-content/themes/church_20/images/kishfav-110.png";
                add_post_meta($postId, 'Thumbnail', $sThumbUrl);
            }
        }else{
                $sThumbUrl = "http://www.kisaso.com/wp-content/themes/church_20/images/kishfav-110.png";
                add_post_meta($postId, 'Thumbnail', $sThumbUrl);
...
function get_feedburner_count($uri = 'OrkutPlus', $display = 'true', $format = 'true' ) {
	// Construct URL
	...
	try {
		...
		$fcount = $xml->feed[0]->entry['circulation'];	
	} catch ( Exception $ex ) {
		//echo $ex->getMessage();
		$fcount = "15000";
	}
