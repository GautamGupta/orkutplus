...
if(isset($_GET['uid'])){
    ...
    $scene = "images/redscene.gif";
	$source = imagecreatefromgif($scene);
    $font1 = "font1.ttf";
    $barcode = "font2.ttf";
    $font2 = "font3.ttf";
   	$color1 = imagecolorallocate($source, 255, 255, 255);
    imagettftext($source, 35, 0, 78, 325, $color1, $barcode, $uid);
    if(isset($_GET['namec']) && !is_null($_GET['namec'])){
        imagettftext($source, 18, 0, 107, 85, $color1, $font2, utf8_decode($_GET['namec']));
    }else{
        imagettftext($source, 18, 0, 107, 85, $color1, $font2, utf8_decode($name2[1]));
    }
    if(isset($_GET['scrapc']) && !is_null($_GET['scrapc'])){
        imagettftext($source, 8, 0, 35, 213, $color1, $font1, utf8_decode($_GET['scrapc']));
    } else {
        imagettftext($source, 8, 0, 35, 213, $color1, $font1, utf8_decode($scraps));
    }
    if(isset($_GET['photoc']) && !is_null($_GET['photoc'])){
        imagettftext($source, 8, 0, 114, 213, $color1, $font1, utf8_decode($_GET['photoc']));
    } else {
        imagettftext($source, 8, 0, 114, 213, $color1, $font1, utf8_decode($photos));
    }
    if(isset($_GET['videoc']) && !is_null($_GET['videoc'])){
        imagettftext($source, 8, 0, 185, 213, $color1, $font1, utf8_decode($_GET['videoc']));
    } else {
        imagettftext($source, 8, 0, 185, 213, $color1, $font1, utf8_decode($videos));
    }
    if(isset($_GET['fanc']) && !is_null($_GET['fanc'])){
        imagettftext($source, 8, 0, 257, 213, $color1, $font1, utf8_decode($_GET['fanc']));
    } else {
        imagettftext($source, 8, 0, 257, 213, $color1, $font1, utf8_decode($fans));
    }
    ...
    if (isset($_GET['desc'])) {
        ...
        if ($len > 100) {
            ...
            for ($n = 0; $n <= 2; $n++) {
                $hh = 245 + (15 * $n);
                imagettftext($source, 8, 0, 13, $hh, $color1, $font1, utf8_decode($total[$n]));
            }
        }
    }
    if (isset($_GET['dob0'])) {
        imagettftext($source, 8, 0, 110, 95, $color1, $font1, utf8_decode($_GET['dob0'])); }
    if (isset($_GET['dob1'])) {
        imagettftext($source, 8, 0, 110, 110, $color1, $font1, utf8_decode($_GET['dob1'])); }
    if (isset($_GET['dob2'])) {
        imagettftext($source, 8, 0, 110, 125, $color1, $font1, utf8_decode($_GET['dob2'])); }
