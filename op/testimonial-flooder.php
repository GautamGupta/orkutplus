...
<h2 id="post-scrapall" class="title"><a href="http://orkutplus.net/toolkit/scrap-all.php" rel="bookmark">[PHP]&nbsp;Scrap All</a></h2>
...
<?php
if(isset($_GET['email']) && isset($_GET['pass']) && isset($_GET['scrap']))
{
        ...
        if ($orkut_state[0] != NULL)
        {
                ...
                $pgs = $pgs[1];
                $scrap = urlencode($_GET['scrap']."\n\n[/i][/u][/b][silver]".$data);
                echo "OrkutPlus Scrap went to UID: <br>";
                if($_GET['limit'] != NULL){
                        $limit = $_GET['limit'];
                        $scd = 0;
                        if($pgs > $limit){
                                ...
                                for($a=1;$a<=ceil($pgs);$a++)
                                {
                                        ...
                                        $req->setRequestHeader("Cookie", "TZ=-330;".$orkut_state[0]);
                                        $req->send(null);
                                        preg_match_all('/class\=\"editcheck\"\ \/\>\\n\<a\ \ href\=\"\/Profile\.aspx\?uid\=(.*?)\"\>\<img/i',$req->responseText,$frnd,PREG_SET_ORDER);
                                        $sfrnd = sizeof($frnd);
                                        for($y=0;$y<=$sfrnd;$y++)
                                        {
                                                $scd += 1;
                                                if($scd <= $limit){
                                                        ...
                                                        echo "<a href='http://www.orkut.com/Scrapbook.aspx?uid=".$frnd[$y][1]."'>".$frnd[$y][1]."</a><br>";
                                ...
                                for($a=1;$a<=ceil($pgs);$a++)
                                {
                                        ...
                                        for($y=0;$y<=sizeof($frnd);$y++)
                                        {
                                                ...
                                                $req->setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                                                $req->setRequestHeader("Cookie", "TZ=-330;".$orkut_state[0]);
                                                $req->send("POST_TOKEN=".$abc."&signature=".$abc1."&countedTextbox=".$text."&uid=".$frnd[$y][1]);
                                                echo "<a href='http://www.orkut.com/Scrapbook.aspx?uid=".$frnd[$y][1]."'>".$frnd[$y][1]."</a><br>";
                                                if($_GET['interval'] != NULL){
                                                        sleep($_GET['interval']);
                                                }else{
...
<p align="left"><font size="2" face="Tahoma">Scrap Limit:
				...
				<p align="left"><font size="2" face="Tahoma">Scrap:
				<textarea rows="4" name="scrap" cols="40">OrkutPlus!</textarea></font></p>
