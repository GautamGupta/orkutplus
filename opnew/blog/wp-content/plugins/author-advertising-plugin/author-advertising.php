...
function kd_authoredit(){
   global $table_prefix, $wpdb, $user_ID, $current_user;
   $wpdb->show_errors();
   $google_values = get_option('kd_author_advertising');
   $table_name = $table_prefix . "author_advertising";
   $action = $_POST['action'];
   if($action == "add" && $current_user->user_level <= 1 && $_POST['user_google'] != NULL && $_POST['custom1'] != NULL) {
   ...
   if($action == "edit" && $current_user->user_level >= 2){
      $user_advertising = $wpdb->escape($_POST['user_google']);
      $user_custom1 = $wpdb->escape($_POST['custom1']);
      $wpdb->query("UPDATE $table_name SET author_advertising='$user_advertising',author_custom1='$edited_1',approve='1' WHERE author_id='$user_ID'");
   }
...
<form method="post">
   ...
   <tr valign="top"><th scope="row">Banner Image (The Image that is shown (480x60px))</th>
   <td><input type="text" name="user_google"></td>
   </tr>
...
<tr class="thead">
   <th>Banner ID</th>
   <th>Banner Image (The Image that is shown (480x60px))</th>
...
<?php
$userresults = $wpdb->get_results("SELECT id, author_id, author_advertising, author_custom1, author_custom2, approve, impressions, clicks FROM $table_name WHERE author_id = '$user_ID'");
...
?>
<br />
<strong>You have stepped up 1 level. You can now enter your Adsense ID here, instead of banners. (<a href="#">More Information</a>)</strong>
   <form method="post">
   <table class="form-table">
...
<?php
   ...
   if($action == "edited"){
      ...
      $approve = stripslashes($_POST['approve']);
      if($_POST['notify'] == "on"){
   ...
   $userresults = $wpdb->get_results("SELECT id, author_id, author_advertising, author_custom1, author_custom2, approve, impressions, clicks FROM $table_name ORDER BY author_id ASC");
?>
<table class="widefat">
<thead>
<tr class="thead">
...
function mail_user_status($approve, $user_id, $banner_id, $admin = 1){
   ...
   if($approve == 2){
	...
	$subject='[Orkut Plus!] Your Ad Banner No. '.$banner_id.' is Approved!';
        $message="Hi {$contributor->display_name},\n\n We have just approved the banner (No. {$banner_id}) that you had uploaded on OrkutPlus <www.orkutplus.net> as a Guest Blogger. Thanks for registering with us. If you have any queries, you can mail us at <REDACTED>@gmail.com.\n\nKeep Blogging!\n\nThank You.\n\n{$sender_name} <{$sender_mail}>";
        wp_mail($contributor->user_email, $subject, $message);
   }elseif($approve == 0){
...
function kd_get_google_id($banner_id){
   global $table_prefix, $wpdb;
   $table_name = $table_prefix . "author_advertising";
   $google_values = get_option('kd_author_advertising');
...
function kd_get_custom1($banner_id){
   global $table_prefix, $wpdb;
   $table_name = $table_prefix . "author_advertising";
   $google_values = get_option('kd_author_advertising');
...
function kd_get_banner_id($user_id){
   global $table_prefix, $wpdb;
   $table_name = $table_prefix . "author_advertising";
   $banner_id = $wpdb->get_var("SELECT id FROM $table_name WHERE author_id='$user_id' and approve='1' ORDER BY rand() LIMIT 1");
   if(!banner_id){ $banner_id = 1; }
   return $banner_id;
}
...
function kd_get_ad_ready($authid = 1){
   global $table_prefix, $wpdb;
   ...
   if($user_info->user_level <= 1){
      ...
      $google_id = kd_get_google_id($banner_id);
      $custom1 = plugins_url('author-advertising-plugin/r.php?i='.$banner_id);
      $custom2 = kd_get_custom2($banner_id);
      $adtext = str_replace("%pubid%", $google_id, $google_values[14]);
      $adtext = str_replace("%custom1%", $custom1, $adtext);
      $adtext = str_replace("%custom2%", $custom2, $adtext);
      $adtext .= "\n".'<!-- Start Of Script Generated By My Ad Banner -->'."\n";
      ...
      $banner_id = kd_get_banner_id($authid);
      $google_id = kd_get_google_id($banner_id);
      $custom1 = kd_get_custom1($banner_id);
      $adtext = str_replace("%pubid%", $google_id, $google_values[16]);
      $adtext = str_replace("%custom1%", $custom1, $adtext);
  }
  return $adtext;
}
